...
@app.post("/asynchttpx")
def input_asynchttpx(request: Request, input: DataRequest):
    try:
...
async def main(base_url, headers, keyword, pages):
    async with httpx.AsyncClient() as session:
        loop = asyncio.get_event_loop()
        tasks = [
            loop.create_task(scrape(base_url, headers, keyword, page, session))
            for page in range(1, pages + 1)
        ]
        product_soup = []
        for task in asyncio.as_completed(tasks):
            soup_produk = await task
...
async def scrape(base_url, headers, keyword, page, session):
...
async def scrape_produk(product_soup, headers, session):
    tasks = [scrape_page(soup, session, headers) for soup in product_soup]
    tasks = await asyncio.gather(*tasks)
    return tasks


async def scrape_page(soup, session, headers):
    ...
    results = []
    results.append(asyncio.create_task(data_product(soup, new_link, session, headers)))
    results.append(
        asyncio.create_task(
...
async def data_product(soup_produk, product_link, session, headers):
    try_count = 0
    while try_count < 5:
        try:
                ...
                if key == "produk_details":
                    ...
                    if elements:
                        for key_element in elements:
                            ...
                            # product_items[kunci] = [
                            #     item.text.strip() + "*habis" if item.find("button", {"class": "css-10qfd4z-unf-chip e6yxrl1"}) or item.find("button", {"class": "css-1xiqghs-unf-chip e6yxrl1"}) else item.text.strip()
                            #     for item in items
                            #     if item.text.strip()
                            #     != ".css-1y1bj62{padding:4px 2px;display:-webkit-inline-box;display:-webkit-inline-flex;display:-ms-inline-flexbox;display:inline-flex;}"
                            # ]
...
async def data_shop(shop_link, session, headers):
    try_count = 0
    while try_count < 5:
        try:
